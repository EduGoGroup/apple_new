Puedes agregar esto a la memroria delproyecto , te la coloco ahora porque te va ayudar en la tarea que te voy a pedir
...
Como veras esta muy adelantado  el proyecto kmp_new, se hizo manejo de UI dinamica, menu dinamico, modo offline, todavia falta cosas por corregir, y hay planes para expandir, pero no quiero que apple_new se quede muy atras, y sabemos que necesitamos aplicar esa logica al estandar de swift ui, usando lo ultimo, en la carpeta /Users/jhoanmedina/source/EduGo/EduUI/kmp_new/docs/flujos-arquitectura se encuentra documentacion, de lo que se ha hecho, y lo que falta por hacer, hay mucha mucha informacion de flujo que me gustaria pasarlo a apple_new

El trabajo es, aparte de analizar todo lo de arriba dado, crear un plan de trabajo muy muy detallado, en fase, y guardar las fases en una carpeta, con un readme.md indicando los pasos a seguir, tu labor ahora, usa un equipo de agentes para hacer el analisis de lo que tenemos, y lo que se debe migrar, recordando siempre, no debemos tener codigo deprecado, ni codigo por retrocompatibilidad, eliminamos y creamos, al final cada fase debe validar que todo compile y todo testee

Si debes modificar alguna de las api, o los codigos de proyectos comunes, recordar crear una rama desde dev, y mediante go.work, hacer las modificaciones necesarias, si necesitas crear un nuevo modulo en los proyectos compartidos, debes actualizar el go,work, tambien si necesitas probar las api con cambios, puedes levantar las api local, como te indique arriba, si hay algun proceso que esta consumiendo el puerto tumba todo proceso, si necesitas probar los endpoint existente, puedes probar con la api en azure, recordando despertarla con el sh

al final tendremos un plan de trabajo muy detallado para mas adelante implementarlo
