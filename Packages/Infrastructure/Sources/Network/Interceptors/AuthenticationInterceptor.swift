...
/// Handler invocado cuando la sesión expira (refresh token falla).
///
/// Permite al layer superior (e.g. Domain/Presentation) reaccionar
/// a la expiración de sesión y redirigir al login.
public protocol SessionExpiredHandler: Sendable {
    /// Invocado cuando un refresh token falla y la sesión se considera expirada.
...
public actor AuthenticationInterceptor: RequestInterceptor {
    ...
    /// Handler opcional para sesión expirada.
    private let sessionExpiredHandler: (any SessionExpiredHandler)?
    ...
    ///   - sessionExpiredHandler: Handler invocado cuando el refresh falla (default: nil)
    ...
    public init(
        ...
        sessionExpiredHandler: (any SessionExpiredHandler)? = nil,
    ...
    public func retry(
        ...
        guard refreshed else {
            ...
            await sessionExpiredHandler?.onSessionExpired()
            return .doNotRetry
        }
...
extension AuthenticationInterceptor {
    ...
    /// - Parameters:
    ///   - tokenProvider: Proveedor de tokens
    ///   - sessionExpiredHandler: Handler para sesión expirada (default: nil)
